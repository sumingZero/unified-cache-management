...
class UCMFAWALiteConnector(UCMLiteConnector):
    ...
    def _compute_fawa_sizes(
        cls, vllm_config: "VllmConfig", kv_cache_config: "KVCacheConfig",
        is_ascend: bool, bs: int,
    ) -> tuple[int, int, int]:
        ...
        if is_ascend:
            ...
            idx_per_tok = 132
            align = 576
            g0 = (num_c4a * round_up(bs // 4 * mla_per_tok, align)
    ...
    def __init__(
        ...
        for gid, group in enumerate(groups):
            ...
            mtype = "compress" if (is_ascend and window is None) else (
                "full_attention" if window is None else "sliding_window"
            )
        ...
        alignment = reduce(lcm, block_sizes_for_gcd) if block_sizes_for_gcd else 1
